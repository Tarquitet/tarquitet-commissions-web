...
import Step3Backgrounds from './calculator/steps/3_Backgrounds';
import Step4YCH from './calculator/steps/4_YCH';
import Step5PSD from './calculator/steps/5_PSD';
import Step6Licenses from './calculator/steps/6_Licenses';
import Step7Summary from './calculator/steps/7_Summary';
...
export default function CommissionModal({ isOpen, onClose }: { isOpen: boolean; onClose: () => void }) {
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/80 backdrop-blur-md p-4">
      <div className="bg-[#0a0000]/90 border border-brand-red/40 w-full max-w-4xl rounded-[2rem] overflow-hidden flex flex-col max-h-[90vh] shadow-[0_0_50px_rgba(220,38,38,0.15)]">
        {/* PROGRESS BAR HEADER */}
        <div className="p-6 border-b border-white/10 flex justify-between items-center bg-gradient-to-r from-brand-red/20 to-transparent shrink-0">
          <div className="flex gap-1.5 flex-1 mr-8">
            {Array.from({ length: totalSteps }).map((_, i) => (
              <div
                key={i}
                className={`h-1.5 flex-1 max-w-[40px] rounded-full transition-all duration-300 ${logic.step >= i + 1 ? 'bg-brand-red shadow-[0_0_10px_rgba(220,38,38,0.8)]' : 'bg-white/10'}`}
              />
            ))}
          </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 sm:p-8 custom-scrollbar">
          {isLoading ? (
            ...
            <>
              ...
              {logic.step === 4 && (
                <Step4YCH ychData={ychData} ychSelection={logic.ychSelection} setYchSelection={logic.setYchSelection} />
              )}
              {logic.step === 5 && (
                <Step5PSD
                  groupedOptions={logic.groupedOptions}
